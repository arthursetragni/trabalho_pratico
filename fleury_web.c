...
/*
    ...
    "Grafos que possuem um circuito Euleriano s�o chamados Grafos Eulerianos.
     Uma das principais condi��es para um grafo ser Euleriano � que todos os v�rtices precisam ser de grau par.
     Entretanto, essa condi��o n�o � suficiente, pois tamb�m � necess�rio que o grafo seja conexo.

     Euler provou que uma condi��o necess�ria para a exist�ncia de circuitos eulerianos � de que todos os v�rtices tenham grau par,
     e afirmou, sem prova de que grafos conexos com todos os v�rtices pares tem um circuito Euleriano.
     A primeira prova completa desta �ltima afirma��o foi publicada em 1873 por Carl Hierholzer.

     H�, ainda, grafos com caminhos Eulerianos se houver 2 v�rtices de grau �mpar.
     Nesse caso, ao se acrescentar uma aresta ligando estes dois v�rtices, o novo grafo passa a ser um circuito Euleriano."
*/
...
void fleury(){
    ...
    int grauNo[qtdVertices]; //vetor com o grau de cada n�
    ...
    for(i = 0; i < qtdVertices; i++){
        //dfs � um algoritmo de busca em profundidade
        ...
        if(qtdVerticesVisitados != qtdVertices - 1){ //se encontrar um vertice que nao tem acesso a todos os outros
            conexo = 0;                              //sinaliza que o grafo nao � conexo
        }

        //Zera variaveis para a proxima intera��o
        memset(visitados, 0, sizeof(visitados));
        qtdVerticesVisitados = 0;
    }

    //verifica�ao de todos os graus
    for(i = 0; i < qtdVertices; i++){
            ...
            if(grauImpar % 2 == 0){ //se encontrou uma dupla de nos com grau par
                ...
                if(matriz[primeiroImpar][segundoImpar] != 0){ //se os dois nos com grau impar possuem liga�ao

                    matriz[primeiroImpar][segundoImpar]--;//retira essa aresta, para verificar se a aresta � uma ponte (verificando se o grafo continua sendo conexo)

                    /******ir� verificar se o grafo continua sendo conexo: ********/
                    ...
                    if(conexo == 1){ //se nao for uma aresta ponte
                        excecao = 1;  //sinaliza que eles tem liga�ao e nao formam ponte, entao est� apto a ter ciclo euleriano
    ...
    /******* ir� mostrar se encontrou erros, se sim, quais ***************************************************************************/
    ...
    if(grauImpar % 2 == 0 && excecao){ //se nao tiver vertice com grau impar, ou se tiver dois nos com grau impar por�m vizinhos e nao ponte
    ...
    /** resposta final **/
    if(conexo && excecao) //se for conexo e todos os nos tem grau par com exce��o de pares de vizinhos com grau impar que nao formam ponte
...
void adjacente(int **matriz, int quantidadeVertices, int *grauNo){
	FILE *f = fopen("grafoEuleriano.txt", "r+");
...
void dfs(int i, int **matriz, int qtdVert, int *visitados, int *qtdVerticesVisitados){
    ...
    for(j = 0; j < qtdVert; j++){
        if(!visitados[j] && matriz[i][j] != 0){   //se nao foi visitado e
                                                  //existe liga�ao entre os vertices i e j
